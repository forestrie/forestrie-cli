...
export default defineForestrieCommand({
  meta: {
    ...
    description:
      "Produce or freshen a COSE receipt (SCRAPI §2.4) — source by flags: --massif (tiles), --receipt + --checkpoint-chain (.sth freshen), or --receipt + --rpc-url/--univocity/--log-id + --checkpoint (calldata freshen). Freshen re-anchors a stale receipt to the current accumulator; the trust posture is set at verify (https://github.com/forestrie/protocol/blob/main/spec/receipt-trust-model.md) [FOR-345/FOR-418]. Alias: create-receipt.",
  },
  args: {
    // --- tiles source (--massif) ---
    massif: {
    ...
    "checkpoint-chain": {
      type: "string",
      description:
        "Retained .sth checkpoint chain — a directory or comma-separated files (freshen source; the freshened receipt is emitted under the chain's own tail checkpoint)",
      valueHint: "dir|files",
    },
    "committed-grant": {
      type: "string",
    ...
    "known-accumulator": {
      type: "string",
      description:
        "Trusted accumulator snapshot (fetch-accumulator output) — binds the freshened state to a trusted chain read, failing closed on mismatch (freshen only; anchors freshness, not signer provenance — https://github.com/forestrie/protocol/blob/main/spec/receipt-trust-model.md)",
      valueHint: "path",
    },
  },
